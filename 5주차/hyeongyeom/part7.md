...
1. `docker warm init [옵션]` 명령을 통해 도커 스웜 초기화를 수행하고 `docker node ls` 명령어를 통해 node 상태를 확인할 수 있습니다.
    
![part7-4](assets/part7-4.png)

    
2. 위 docker swarm init 명령어를 통해 생성된 명령어  `docker swarm join --token SWARM_TOKEN <MANAGER_IP>:2377` 를 그대로 워커로 설정할 노드에 따라 치거나 복사 붙여넣기 합니다.
    
![part7-5](assets/part7-5.png)
    

1. 간단하게 도커 스웜 클러스터에 매니저 노드와 워커 노드 연결을 설정하였습니다. 이는 Docker Web 관리 Tool ****portainer를 통해 시각적으로 확인할 수 있습니다.
...
1. 각 노드에서 docker ps 명령어를 통해 nignx 컨테이너가 잘 생성되었는지 확인합니다.
...
1. 각 노드에 index.html 을 만들고 nignx 컨테이너 index.html 파일 경로에 복사합니다.
...
1. 클러스터의 어떤 노드에서든 아래와 같이 조회를 수행해 보면 배포된 서비스 컨테이너에 로드 밸런싱으로 교차 접근하는 것을 확인할 수 있습니다.
